...
import type { Companie, Info, User } from "../types";
...
interface ClientCardProps {
  ...
  onUpdate: (id: number, data: { name: string; imgSrc: string }) => Promise<void>;
  onDelete: (id: number) => Promise<void>;
}

const ClientCard = ({ client, isAdmin, onUpdate, onDelete }: ClientCardProps) => {
...
interface AddClientCardProps {
  onAdd: (client: { name: string; imgSrc: string }) => Promise<void>;
}

const AddClientCard = ({ onAdd }: AddClientCardProps) => {
...
export default function AboutPage({ user }: AboutPageProps) {
  ...
  const [clients, setClients] = useState<Companie[]>([]);
  ...
  const fetchAboutData = useCallback(async () => {
    try {
      ...
      const [aboutRes, companiesRes] = await Promise.all([
        axiosInstance.get<Info>("/about"),
        axiosInstance.get<Companie[]>("/companies"),
      ]);
  ...
  const handlerUpdateField = async (field: AboutField, value: string) => {
    try {
      const res = await axiosInstance.put<Info>("/about", { [field]: value });
      if (res.status === 200) {
  ...
  const handleUpdateClient = async (
    id: number,
    updatedData: { name: string; imgSrc: string }
  ) => {
    try {
